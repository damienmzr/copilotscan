...
def _rule_inactive(agent: Agent, inactivity_days: int) -> Optional[RiskFlag]:
    ...
    if agent.purview_last_interaction is None:
        return RiskFlag(
            ...
            message_en=(
                f"No Purview activity recorded for this agent "
    ...
    if agent.purview_last_interaction < cutoff:
        ...
        return RiskFlag(
            ...
            level=RiskLevel.MEDIUM,
            message_en=(
                f"Agent last active {days_ago} days ago "
                f"(threshold: {inactivity_days} days)."
            ),
            message_fr=(
...
def _rule_orphan(agent: Agent) -> Optional[RiskFlag]:
    ...
    if agent.is_org_scoped and agent.publisher is None:
        return RiskFlag(
            ...
            message_en=(
                "Agent is shared org-wide but has no registered publisher / owner."
            ),
            message_fr=(
...
def _rule_sensitive_knowledge(agent: Agent) -> Optional[RiskFlag]:
    ...
    if flagged:
        ...
        return RiskFlag(
            ...
            level=RiskLevel.HIGH,
            message_en=(
                f"Agent accesses potentially over-broad knowledge sources: {sources_str}"
            ),
            message_fr=(
...
def _rule_knowledge_unknown(agent: Agent) -> Optional[RiskFlag]:
...
def _rule_agent_not_audited(agent: Agent, origin: AgentOrigin) -> Optional[RiskFlag]:
...
_ORIGIN_RISK_MSG_EN: dict[AgentOrigin, str] = {
    AgentOrigin.AGENT_BUILDER:
        "Agent was created by an individual user via Agent Builder and shared org-wide.",
    AgentOrigin.SHAREPOINT_AGENT:
        "Agent is a SharePoint-native agent — audit coverage is limited.",
    AgentOrigin.COPILOT_STUDIO:
        "Agent was deployed via Copilot Studio — verify governance approval.",
    AgentOrigin.PRO_CODE:
        "Agent is a custom-engine (pro-code) agent — code review recommended.",
    AgentOrigin.MICROSOFT_PREBUILT:
        "Agent is a Microsoft prebuilt agent — low risk.",
    AgentOrigin.UNKNOWN:
        "Agent origin could not be determined from available metadata.",
}

_ORIGIN_RISK_MSG_FR: dict[AgentOrigin, str] = {
    AgentOrigin.AGENT_BUILDER:
        "L'agent a été créé par un utilisateur individuel via Agent Builder et partagé à l'organisation.",
    AgentOrigin.SHAREPOINT_AGENT:
        "L'agent est natif SharePoint — la couverture d'audit est limitée.",
    AgentOrigin.COPILOT_STUDIO:
        "L'agent a été déployé via Copilot Studio — vérifiez l'approbation de gouvernance.",
    AgentOrigin.PRO_CODE:
        "L'agent est un agent custom-engine (pro-code) — une revue de code est recommandée.",
    AgentOrigin.MICROSOFT_PREBUILT:
        "L'agent est un agent Microsoft préintégré — risque faible.",
    AgentOrigin.UNKNOWN:
        "L'origine de l'agent n'a pas pu être déterminée à partir des métadonnées disponibles.",
}
